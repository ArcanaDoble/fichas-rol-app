    ...
    return (
        <div className="w-full h-full overflow-y-auto custom-scrollbar bg-[#09090b]">
            <div className="max-w-5xl mx-auto p-8 lg:p-12">
                <div className="flex items-center justify-between mb-12 border-b border-[#c8aa6e]/20 pb-6">
                    <div>
                        <h2 className="text-4xl font-['Cinzel'] text-[#f0e6d2] mb-2">CONSTELACIÓN</h2>
                        <p className="text-slate-400 text-sm uppercase tracking-widest">Progreso de Nivel y Rasgos</p>
                    </div>
                    <div className="text-right">
                        <div className="text-3xl font-bold text-[#c8aa6e]">{dndClass.currentLevel} <span className="text-slate-600 text-xl">/ 20</span></div>
                        <div className="text-xs text-slate-500 uppercase font-bold">Nivel Actual</div>
                    </div>
                </div>

                <div className="relative space-y-0">
                    {/* Vertical connecting line */}
                    <div className="absolute left-[35px] top-8 bottom-8 w-[2px] bg-slate-800 z-0"></div>
                    <div
                        className="absolute left-[35px] top-8 w-[2px] bg-gradient-to-b from-[#c8aa6e] to-slate-800 z-0 transition-all duration-1000"
                    ...
                    {Array.from({ length: totalLevels }, (_, i) => i + 1).map((level) => {
                        ...
                        return (
                            <div key={level} className="relative z-10 group mb-6 last:mb-0">
                                <div className={`
                                flex items-center gap-6 p-1 rounded-lg transition-all duration-300
                                ${isCurrent ? 'bg-gradient-to-r from-[#c8aa6e]/10 to-transparent border-l-4 border-[#c8aa6e] pl-4' : ''}
                            `}>
                                    {/* Level Node */}
                                    <div className="flex flex-col items-center shrink-0">
                                        <div className={`
                                        w-16 h-16 rounded-full flex items-center justify-center text-xl font-bold font-['Cinzel'] border-4 relative bg-[#0b1120] shadow-xl transition-all duration-300
                                    ...
                                    {/* Content Box */}
                                    <div className={`
                                    flex-1 p-5 rounded border backdrop-blur-sm transition-all duration-300 flex items-center gap-6
                                    ${isPast
                                                    ...
                                                    {features.map((feature, idx) => (
                                                        <div key={idx} className="relative">
                                                            <EditableText
                                                                ...
                                                                className={`font-bold text-lg font-['Cinzel'] mb-1 block ${isPast ? 'text-[#f0e6d2]' : 'text-slate-400'}`}
                                                            />
                                                            <EditableText
