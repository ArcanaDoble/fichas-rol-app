...
const KarmaBar = ({ value, min = KARMA_MIN, max = KARMA_MAX, className = '' }) => {
  ...
  return (
    <div
      className={`relative h-4 w-full overflow-hidden rounded-full bg-gray-700/80 ${className}`}
    >
      <div className="absolute inset-y-0 left-1/2 w-px bg-gray-500/60" />
      <div
        className="absolute inset-y-0 left-1/2 bg-white transition-all duration-300 ease-out"
        style={{ width: positiveWidth, borderRadius: '0 9999px 9999px 0' }}
      />
      <div
        className="absolute inset-y-0 right-1/2 bg-black transition-all duration-300 ease-out"
      ...
      <div className="absolute inset-0">
        <div className="grid h-full w-full grid-cols-[repeat(20,_minmax(0,1fr))] opacity-40">
          {Array.from({ length: 20 }).map((_, index) => (
            <div key={index} className="border-r border-gray-600/40 transition-all duration-300" />
          ))}
        </div>
      </div>
    </div>
