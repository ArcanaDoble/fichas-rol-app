...
import { isCombatTokenItem, normalizeTokenStatusIds } from '../../legacyCombatRules';
import { CardImageWithLoader, TokenImageWithLoader } from '../TacticalAssetImage';
import { EquipmentSection as LegacyEquipmentSection } from '../EquipmentSection';
...
export const TokenInspectorPanel = ({
    ...
    BoardDieVisual,
    BoardMarkerVisual,
    EquipmentSectionComponent = LegacyEquipmentSection,
    TokenResourcesComponent = LegacyTokenResources,
    PRESET_COLORS,
    accesorios,
    activeScenario,
    activeTab,
                                                    ...
                                                    <div className="pt-4 border-t border-slate-800/50 space-y-4">
                                                        ...
                                                        {(isMaster || (token.controlledBy?.includes(playerName) && playerName)) && (
                                                            <div className="space-y-3 p-3 bg-slate-800/20 rounded border border-slate-800/40">
                                                                ...
                                                                {(token.linkedCharacterId || token.linkedClassId) ? (
                                                                        ...
                                                                        <div className="flex items-center gap-2.5 overflow-hidden">
                                                                            ...
                                                                            <div className="flex flex-col min-w-0">
                                                                                <span className="text-[11px] text-[#f0e6d2] truncate font-bold uppercase tracking-wider">
                                                                                    {availableCharacters?.find(c => c.id === token.linkedCharacterId)?.name || token.name || 'Archivo Vinculado'}
                                                                                </span>
                                                                                <span className="text-[8px] text-slate-500 font-bold uppercase">
                                                                                    {token.linkedClassId ? 'Clase sincronizada' : 'Sincronizado'}
                                                                                </span>
                                                                            </div>
                                                                        </div>
                                                                        {/* Solo permitir desvincular si eres el Master o el dueño de esa ficha específica */}
                                                                        {token.linkedCharacterId && (isMaster || availableCharacters.some(c => c.id === token.linkedCharacterId)) && (
                                                                            <button
                                                                ...
                                                                {!isBoardMode && token.linkedClassId && token.runtimeDirty && (
                                                                    <div className="flex items-center gap-2 border-t border-[#c8aa6e]/15 pt-2" data-testid="roguelite-pending-changes">
                                                                        <span className="h-1.5 w-1.5 shrink-0 bg-amber-300" />
                                                                        <span className="text-[9px] font-bold uppercase tracking-[0.16em] text-amber-200/85">
                                                ...
                                                {(isCombatTokenItem(token)) && (() => {
                                                    ...
                                                    return (
                                                        <EquipmentSectionComponent
                                                            ...
                                                            onUpdateToken={(updates, persist = false) => updateItem(token.id, updates, persist)}
                                                            onAddItem={(item, type) => {
