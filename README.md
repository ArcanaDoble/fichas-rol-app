...
- [API pública del adaptador PixiBattleMap](docs/PixiBattleMapAPI.md)
...
- **Redimensionado con asas en las esquinas** - Al seleccionar un token aparecen cuatro tiradores que permiten ajustar su tamaño con arrastre, ahora con incrementos de 0,25 casillas por debajo de 1×1 y de 1 casilla a partir de ahí, manteniendo el token centrado incluso en tamaños pares. Mantén **Alt** para desactivar temporalmente el snap, usa **Shift** para avanzar paso a paso y haz doble clic en una asa para ciclar entre 1×1, 2×2 y 3×3. Durante el arrastre se muestra la medida actual en una etiqueta flotante y el nuevo valor se sincroniza con el chat y el portapapeles
...
# La instalación respeta la configuración de `.npmrc`, que habilita el modo
# `legacy-peer-deps` para combinar `pixi.js@^7` con `pixi-viewport@^6` sin
